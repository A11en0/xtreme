...
def try_remove_special_token(tokenizer, tokens):
  if len(tokens) == 0:
    return tokens
  special_tag = "▁"
  if tokens[0] == special_tag:
    tokens = tokens[1:]
  elif tokens[0][0] == special_tag:
    new_token = tokens[0][1:]
    if tokenizer.convert_tokens_to_ids([new_token])[0] != tokenizer.unk_token:
      tokens[0] = new_token
  return tokens
      ...
      if isinstance(tokenizer, XLMTokenizer):
        ...
        if isinstance(tokenizer, XLMRobertaTokenizer):
          if remove_special_token:
            word_tokens = try_remove_special_token(tokenizer, word_tokens)
